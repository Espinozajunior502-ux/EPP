...
async function pdfRegistroIndividual(id){
  ...
  doc.text('Verificación de EPP — Megablessing S.C.', 15, y); y+=8;
...
async function pdfResumenDia(fecha){
  ...
  let y=18;
  doc.setFont('helvetica','bold'); doc.setFontSize(15);
  doc.text('Resumen diario de verificación EPP — Megablessing S.C.', 15, y); y+=7;
  doc.setFontSize(11); doc.setFont('helvetica','normal');
  doc.text(`Fecha: ${fmtFecha(fecha)}`, 15, y); y+=8;
